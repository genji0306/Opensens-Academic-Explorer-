...
def build_crystal_mof(data, niggli=True, primitive=False):
    parameters = data['parameters']
    atom_type = data['atom_type']
    frac_coords = data['atom_frac_pos']
    crystal = Structure(
...
def process_one_mof(row, niggli, primitive, graph_method, prop_list, use_space_group = False, tol=0.01):
    crystal = build_crystal_mof(
        row, niggli=niggli, primitive=primitive)
    result_dict = {}
    if use_space_group:
        crystal, sym_info = get_symmetry_info(crystal, tol = tol)
        result_dict.update(sym_info)
...
def preprocess_mof(input_file, num_workers, niggli, primitive, graph_method,
               prop_list, use_space_group = False, tol=0.01):
    data_set = LMDBDataset(input_file)

    unordered_results = p_umap(
        process_one_mof,
        [data_set[idx] for idx in range(len(data_set))],
        [niggli] * len(data_set),
        [primitive] * len(data_set),
        [graph_method] * len(data_set),
        [prop_list] * len(data_set),
        [use_space_group] * len(data_set),
        [tol] * len(data_set),
        num_cpus=num_workers)

    # with Pool(num_workers) as pool:
    #     process_one_mof_p = partial(process_one_mof, niggli=niggli, primitive=primitive, graph_method=graph_method, prop_list=prop_list, use_space_group=use_space_group, tol=tol)
    #     unordered_results = pool.imap_unordered(process_one_mof_p, [data_set[idx] for idx in range(len(data_set))])
    
    mpid_to_results = {result['mp_id']: result for result in tqdm(unordered_results)}
    ordered_results = [mpid_to_results[data_set[idx]['id']]
