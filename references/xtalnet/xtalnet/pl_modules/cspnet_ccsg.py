...
from xtalnet.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPPXRDNet(nn.Module):
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, pxrd_feat):
        ...
        if t is None:
            ...
            pxrd_per_atom = pxrd_feat[node2graph]
...
class CSPPXRDAddNet(nn.Module):
    ...
    def __init__(
        ...
        super(CSPPXRDAddNet, self).__init__()
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, pxrd_feat):
        ...
        # split frac_coords by num_atoms
        frac_coords = torch.split(frac_coords, num_atoms.tolist(), dim=0)
        # cat (0.5, 0.5, 0.5) for each frac_coords group
        frac_coords = [torch.cat([x, torch.ones_like(x[:1, :]) * 0.5], dim=0) for x in frac_coords]
        frac_coords = torch.cat(frac_coords, dim=0)
        node2graph = torch.split(node2graph, num_atoms.tolist(), dim=0)
        node2graph = [torch.cat([x, torch.ones_like(x[:1]) * i], dim=0) for i, x in enumerate(node2graph)]
        node2graph = torch.cat(node2graph, dim=0)
        edges, frac_diff = self.gen_edges(num_atoms, frac_coords, lattices, node2graph)
        edge2graph = node2graph[edges[0]]
        if self.smooth:
            node_features = self.node_embedding(atom_types)
            ...
            node_features = self.node_embedding(atom_types - 1)
        node_features = torch.split(node_features, num_atoms.tolist(), dim=0)
        # cat pxrd_feat for each node_features group
        node_features = [torch.cat([node_features[i], pxrd_feat[i].reshape(1,-1)], dim=0) for i in range(len(node_features))]
        ...
        coord_out = torch.split(coord_out, num_atoms.tolist(), dim=0)
        coord_out = [x[:-1] for x in coord_out]
        coord_out = torch.cat(coord_out, dim=0)
        graph_features = scatter(node_features, node2graph, dim = 0, reduce = 'mean')
        lattice_out = self.lattice_out(graph_features)
        lattice_out = lattice_out.view(-1, 3, 3)
        if self.ip:
